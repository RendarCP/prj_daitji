...
export default function ExplorerClient() {
  ...
  // React Query hooks
  const { data: locations = [], isLoading: isLoadingLocations } = useLocations({ tree: true });
  const { data: subLocations = [] } = useLocations({ parent_id: selectedLocationId });
  ...
  const breadcrumbItems: ({ id: string; name: string; isHome?: boolean } | Location)[] = [
    { id: "root", name: "Home", isHome: true },
    ...breadcrumbPath,
  ];
  ...
  return (
    <div className="min-h-screen bg-background pb-20">
      {/* Centered Page Title & Back Button */}
      <div className="pt-6 pb-2 text-center relative px-4">
        {selectedLocationId && (
          <button 
            onClick={() => {
              if (breadcrumbPath.length > 1) {
                const parentId = breadcrumbPath[breadcrumbPath.length - 2].id;
                handleLocationSelect({ id: parentId } as Location);
              } else {
                handleLocationSelect({ id: null } as any);
              }
            }}
            className="absolute left-4 top-1/2 -translate-y-1/2 p-2 rounded-full hover:bg-secondary/80 transition-colors"
          >
            <ArrowLeft className="w-5 h-5 text-muted-foreground" />
          </button>
        )}
        <h1 className="text-xl font-semibold text-foreground">Location Explorer</h1>
      </div>

      {/* Breadcrumb Navigation */}
      <div className="px-4 py-2 flex items-center justify-center gap-2 overflow-x-auto no-scrollbar mb-6">
      ...
      <div className="container mx-auto px-4 max-w-3xl space-y-6">
        ...
        <div>
          <div className="flex items-center justify-between mb-3 px-1">
            ...
            <button
              onClick={() => router.push(`/explorer/add${selectedLocationId ? `?parent_id=${selectedLocationId}` : ''}`)}
          ...
          {isLoadingLocations ? (
            ...
            <div className="card p-6 text-center bg-secondary/20 border-border/50 border-dashed">
        ...
        {selectedLocationId && (
          <div className="pb-20">
            <div className="flex items-center justify-between mb-3 px-1">
              <h2 className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">
                Items in {breadcrumbPath.length > 0 ? breadcrumbPath[breadcrumbPath.length - 1].name : "Current Location"}
              </h2>
            ...
            {isLoadingItems ? (
              ...
              <div className="flex flex-col items-center justify-center py-12 text-center opacity-60">
                ...
                <p className="text-sm text-muted-foreground font-medium">No items here yet</p>
              </div>
            ) : (
              <div className="space-y-2">
                {filteredItems.map((item: Item) => {
                  const daysUntilExpiry = item.days_until_expiry;
                  const isExpired = daysUntilExpiry !== null && daysUntilExpiry !== undefined && daysUntilExpiry < 0;
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex-1 text-left min-w-0">
                          <div className="flex items-center gap-2 mb-0.5">
                            ...
                            {isExpiring && (
                              <Badge variant="warning" size="sm" className="h-5 px-1.5 text-[10px]">
                                Expiring
                              </Badge>
                            )}
                          </div>
      ...
      <QuickAddButton 
        ...
        onAddLocation={() => router.push(`/explorer/add${selectedLocationId ? `?parent_id=${selectedLocationId}` : ''}`)}
      />
